...
$ExportCSV=".\$client MFADisabledUserReport_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv"
$ExportCSVReport=".\$client MFAEnabledUserReport_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv"
...
Get-MsolUser -All | foreach{
 ...
 $RolesAssigned=""
 Write-Progress -Activity "`n     Processed user count: $UserCount of $Totalusers"`n"  Currently Processing: $DisplayName" -PercentComplete ($UserCount/$Totalusers*100)
...
if((Test-Path -Path $ExportCSV) -eq "True")
{
 ...
 $UserInput = $Prompt.popup("Do you want to open the disabled user report?",`
...
if((Test-Path -Path $ExportCSVReport) -eq "True")
{
 ...
 $Prompt = New-Object -ComObject wscript.shell
 $UserInput = $Prompt.popup("Do you want to open the enabled user report?",`
 0,"Open Output File",4)
 If ($UserInput -eq 6)
 {
